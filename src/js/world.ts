...
import { color, point3, vec3, vec3AllocatorScope, vec3Len, vec3Rand, vec3RandMinMax, vec3Sub2 } from './vec3';
...
vec3AllocatorScope(new ArenaVec3Allocator(4096), () => {
    ...
    for (let a = -11; a < 11; a++) {
        for (let b = -11; b < 11; b++) {
            ...
            const center = point3(a + 0.2 + 0.6 * Math.random(), 0.2, b + 0.2 + 0.6 * Math.random());
            if (vec3Len(vec3Sub2(center, point3(4, 0.2, 0))) > 0.9) {
                ...
                grid.addHittable(a + 11, b + 11, new Sphere(center, 0.2, sphere_mat));
            }
        }
    }
