...
import {
    ...
    vec3,
    vec3_add_2,
    vec3_allocator_scope_async,
    vec3_rand_min_max2
} from '../math/vec3';
import { solid_color } from '../texture/solid_color';
import { Box } from '../hittable/box';
import { get_predefined_random, random_min_max, random_scope_async } from '../math/random';
...
export const book2_final_scene = async (scene_creation_random_numbers: number[]): Promise<Scene> => {
    ...
    return random_scope_async(rng, () => {
        return vec3_allocator_scope_async(new ArenaVec3Allocator(1024 * 6), async (): Promise<Scene> => {
